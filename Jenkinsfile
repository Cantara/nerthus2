            ...
            steps {
                script {
                    ...
                    if (release) {
                        ...
                        sh "docker buildx build --push --platform linux/arm64,linux/amd64 --tag cantara/nerthus:latest .
                    } else {
...
def buildApp(outFile, vers) {
    ...
    sh "docker buildx build --platform=linux/amd64,linux/arm64 --build-arg BUILDFLAGS=\"${buildFlags}\" -t cantara/nerthus:latest --push ."
