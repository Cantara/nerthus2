            ...
            steps {
                script {
                    ...
                    if (release) {
                        ...
                        sh "cd probe/health && find . -name '${outFile}-*' -type f -exec curl -v -u "+'$NEXUS_CREDS'+" --upload-file {} https://mvnrepo.cantara.no/content/repositories/releases/no/cantara/gotools/${artifactId}/${vers}/probe/health/{}  \\;"
                    } else {
                        ...
                        sh "cd probe/health && find . -name '${outFile}-*' -type f -exec curl -v -u "+'$NEXUS_CREDS'+" --upload-file {} https://mvnrepo.cantara.no/content/repositories/snapshots/no/cantara/gotools/${artifactId}/${vers}/probe/health/{}  \\;"
                    }
                    sh "rm ${outFile}-*"
...
def buildApp(outFile, vers) {
    ...
    sh "cd probe/health && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o ${outFile}-linux-amd64"
    sh "cd probe/health && CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o ${outFile}-linux-arm64"
    sh "cd probe/health && CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o ${outFile}-darwin-amd64"
    sh "cd probe/health && CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o ${outFile}-darwin-arm64"
}
